...
class ZhanDaYeIpSpider extends HtmlSpider
{
    /**
     * 一级页面地址
     *
    ...
    {
        return [
            sprintf("https://www.zdaye.com/dayProxy/%d/%d/1.html", date("Y"), date("m"))
        ];
    }
    ...
    public function getSecondPageUrls()
    {
        ...
        foreach ($this->getUrls() as $page_url) {
            $links = $ql->get($page_url)->absoluteUrl('https://www.zdaye.com')->find('.title a')->attrs('href');
